...
var subject_arr = "";
var part_subject_arr = "";
var child_no1_arr = "";
var child_no2_arr = "";
var child_no3_arr = "";
var part_regis_fee_arr = "";
var part_food_amt_arr = "";
var subject_nm_arr = "";
var regis_fee_arr = "";
var food_amt_arr = "";
...
var akmem_sApprovNo = "";
var akmem_use_sApprovNo = ""; //마일리지 사용요청 결과
var akmem_card_status = "";
var akmem_cust_no = "";
...
function openMCouponLayer()
{
	...
	$("#mcouponLayer_final_pay").val(comma(eval(nullCheck(final_pay))));
}
function openCouponLayer()
{
	if(total_food_amt > 0)
	{
		alert("상품권으로 재료비를 받을 수 없습니다.");
	}
	else if(cash_approve_no != "")
	{
		alert("현금영수증 승인 후엔 상품권을 받을 수 없습니다.");
	}
	else
	{
...
{
	$("#cashLayer_deduction1").prop('checked', true);
	$("#cashLayer_select_fg1").prop('checked', true);
	$("#cashLayer_key_value").val('');
	$("#cashLayer_cash_amt").val('0');
	$("#cashLayer_approve_no").val('');
	$('#cash_layer').fadeIn(200);
	var ls_cash_amt = Number($("#cash_amt").val().replace(/,/gi, ""));
	var change = $("#change").val();
	cash_amt = Number($("#cash_amt").val().replace(/,/gi, ""));
	
	if ((ic_card_co_origin_seq == "555") || (ic_card_co_origin_seq == "666")) {
		ls_cash_amt = eval(nullCheck(cash_amt)) 
		         + eval(nullCheck(coupon_amt))	 
				 + eval(nullCheck(mcoupon_amt))  
				 + eval(nullCheck(card_amt))
				 - eval(nullCheck(change))
	...
	$("#cashLayer_cash_amt").val(comma(ls_cash_amt));
}
//현금승인시 자동공제 버튼클릭시 호출
...
function cash_approveCheck() {
	...
	var 	cardLen = "";
	var amt			= $("#cashLayer_cash_amt").val().replace(/,/gi, "").toString();
	...
	if(key_value == "")
	{
		key_value = $("#cashLayer_key_value").val();
	}
	ls_data1 = key_value;
...
//	}
	if(isEmpty == "N")
	{
		...
		$("[name='couponLayer_face_amt']").each(function() 
		{
			face_amt_arr += $(this).val().replace(/,/gi, "")+"|";
		});
		$("[name='couponLayer_cashrec_yn']").each(function() 
		{
		...
		$("[name='couponLayer_cp_chage_amt']").each(function() 
		{
			cp_chage_amt_arr += $(this).val().replace(/,/gi, "")+"|";
		});
		$("[name='couponLayer_cp_chage_apy_y_amt']").each(function() 
		{
			cp_chage_apy_y_amt_arr += $(this).val().replace(/,/gi, "")+"|";
		});
		$("[name='couponLayer_cp_chage_apy_n_amt']").each(function() 
		{
			cp_chage_apy_n_amt_arr += $(this).val().replace(/,/gi, "")+"|";
		});
		$("[name='couponLayer_cashrec_amt']").each(function() 
		{
			cashrec_amt_arr += $(this).val().replace(/,/gi, "")+"|";
		});
		$("[name='couponLayer_cashrec_n_amt']").each(function() 
		{
			cashrec_n_amt_arr += $(this).val().replace(/,/gi, "")+"|";
		});
	}
...
function eventPayFg() {
    ...
    var coupon_cnt = document.getElementsByName("couponLayer_coupon_fg").length;
	if(coupon_cnt > 0) 
	{
		if(remainAmt == 0)
		{
		...
		*******************************************************************************************/
		...
		{   
			...
			{
				couponAmt += nullCheck(document.getElementsByName("couponLayer_face_amt")[i].value);
			}
			...
			for(var i = 0; i < coupon_cnt; i++)
			{
				if( document.getElementsByName("couponLayer_face_amt")[i].value != null
				    && document.getElementsByName("couponLayer_face_amt")[i].value != ""
					&& document.getElementsByName("couponLayer_coupon_no")[i].value != null
				    && document.getElementsByName("couponLayer_coupon_no")[i].value != ""
				  )
				{
					var faceAmt      = document.getElementsByName("couponLayer_face_amt")[i].value.replace(/,/gi, "");
					var cashrec_yn   = document.getElementsByName("couponLayer_cashrec_yn")[i].value.replace(/,/gi, "");
					var vat_cal_rate = document.getElementsByName("couponLayer_vat_cal_rate")[i].value.replace(/,/gi, "");
					...
					*******************************************************************/
					...
					document.getElementsByName("couponLayer_cp_chage_amt")[i].value = 0;
					document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value = 0;
					document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value = 0;	
					document.getElementsByName("couponLayer_cashrec_n_amt")[i].value = 0;
					document.getElementsByName("couponLayer_cashrec_amt")[i].value = 0;
					...
					if(faceAmt >= 0)
					{
						...
						{ 
							...
							document.getElementsByName("couponLayer_cashrec_n_amt")[i].value = comma(cashrecNAmt);
							document.getElementsByName("couponLayer_cashrec_amt")[i].value = comma(cashrecYAmt);
						} //이외 현금영수증 100% 적용
						else
						{
							document.getElementsByName("couponLayer_cashrec_n_amt")[i].value = 0;
							document.getElementsByName("couponLayer_cashrec_amt")[i].value = comma(faceAmt);

							//alert("[현금영수증100]vat_cal_rate[" + document.getElementsByName("couponLayer_vat_cal_rate")[i].value + "] faceAmt[" + faceAmt 
							//+ "] cashrecNAmt[" + document.getElementsByName("couponLayer_cashrec_n_amt")[i].value 
							//+ "] cashrecYAmt[" + document.getElementsByName("couponLayer_cashrec_amt")[i].value + "]");
						}
					}
					else if(faceAmt < 0)
					{
						...
						//alert("[faceAmt <0] vat_cal_rate[" + document.getElementsByName("couponLayer_vat_cal_rate")[i].value + "] faceAmt[" + faceAmt 
						//+ "] cashrecNAmt[" + document.getElementsByName("couponLayer_cashrec_n_amt")[i].value 
						//+ "] cashrecYAmt[" + document.getElementsByName("couponLayer_cashrec_amt")[i].value + "]");
					}										
					...
					if( remainAmt < 0 )
					{
						document.getElementsByName("couponLayer_cp_chage_amt")[i].value = comma(remainAmt);
						...
						if(faceAmt >= 0)
						{
							...
							{ 
								var cashrecYAmt = parseInt(parseInt(faceAmt * (vat_cal_rate/100)) * 0.1) * 10 ;
								var cashrecNAmt = parseInt(faceAmt - cashrecYAmt);
								var reCashrecYAmt = document.getElementsByName("couponLayer_cashrec_amt")[i].value.replace(/,/gi, "");
								...
								if(i == 0 && cashrecYAmt >= 0 )
								{
									...
									document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value = comma(cashrecNAmt);
									document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value = comma(cashrecYAmt);
								
								//alert("[거스름부가세비율반영1]cp_chage_apy_n_amt[" + document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value
								//     + "] cp_chage_apy_y_amt[" + document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value +"]");												
								}
								...
								//alert("[거스름부가세비율반영1-1]cp_chage_apy_n_amt[" + document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value
								//      + "] cp_chage_apy_y_amt[" + document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value +"]");

									c_coupon_fg          = document.getElementsByName("couponLayer_coupon_fg")[i].value;
									c_coupon_cd          = document.getElementsByName("couponLayer_coupon_cd")[i].value;
									c_coupon_no          = document.getElementsByName("couponLayer_coupon_no")[i].value;
									c_face_amt           = document.getElementsByName("couponLayer_face_amt")[i].value.replace(/,/gi, "");
									c_cashrec_yn         = document.getElementsByName("couponLayer_cashrec_yn")[i].value;
									c_vat_cal_ext_rate   = document.getElementsByName("couponLayer_vat_cal_ext_rate")[i].value;
									c_vat_cal_rate       = document.getElementsByName("couponLayer_vat_cal_rate")[i].value;
									c_cp_chage_amt       = document.getElementsByName("couponLayer_cp_chage_amt")[i].value.replace(/,/gi, "");
									c_cp_chage_apy_y_amt = document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value.replace(/,/gi, "");
									c_cp_chage_apy_n_amt = document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value.replace(/,/gi, "");
									c_cashrec_amt        = document.getElementsByName("couponLayer_cashrec_amt")[i].value.replace(/,/gi, "");	
									c_cashrec_n_amt      = document.getElementsByName("couponLayer_cashrec_n_amt")[i].value.replace(/,/gi, "");	
									...
									document.getElementsByName("couponLayer_coupon_fg")[i].value          = document.getElementsByName("couponLayer_coupon_fg")[0].value;
									document.getElementsByName("couponLayer_coupon_cd")[i].value         = document.getElementsByName("couponLayer_coupon_cd")[0].value;
									document.getElementsByName("couponLayer_coupon_no")[i].value          = document.getElementsByName("couponLayer_coupon_no")[0].value;
									document.getElementsByName("couponLayer_face_amt")[i].value           = document.getElementsByName("couponLayer_face_amt")[0].value;
									document.getElementsByName("couponLayer_cashrec_yn")[i].value         = document.getElementsByName("couponLayer_cashrec_yn")[0].value;
									document.getElementsByName("couponLayer_vat_cal_ext_rate")[i].value   = document.getElementsByName("couponLayer_vat_cal_ext_rate")[0].value;
									document.getElementsByName("couponLayer_vat_cal_rate")[i].value       = document.getElementsByName("couponLayer_vat_cal_rate")[0].value;
									document.getElementsByName("couponLayer_cp_chage_amt")[i].value       = document.getElementsByName("couponLayer_cp_chage_amt")[0].value;
									document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value = 0;
									document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value = 0;
									document.getElementsByName("couponLayer_cashrec_amt")[i].value        = document.getElementsByName("couponLayer_cashrec_amt")[0].value;
									document.getElementsByName("couponLayer_cashrec_n_amt")[i].value      = document.getElementsByName("couponLayer_cashrec_n_amt")[0].value;
									
									
								
									/*
									alert("[거스름부가세비율반영3]gridCoupon.valueMatrix(i, gridCoupon.colRef(coupon_fg))[" + document.getElementsByName("couponLayer_coupon_fg")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(coupon_cd))[" + document.getElementsByName("couponLayer_coupon_cd")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(coupon_no))[" + document.getElementsByName("couponLayer_coupon_no")[i].value 
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(face_amt))[" + document.getElementsByName("couponLayer_face_amt")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(cashrec_yn))[" + document.getElementsByName("couponLayer_cashrec_yn")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(vat_cal_ext_rate))[" + document.getElementsByName("couponLayer_vat_cal_ext_rate")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(vat_cal_rate))[" + document.getElementsByName("couponLayer_vat_cal_rate")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(cp_chage_amt))[" + document.getElementsByName("couponLayer_cp_chage_amt")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(cp_chage_apy_y_amt))[" + document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(cp_chage_apy_n_amt))[" + document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(cashrec_amt))[" + document.getElementsByName("couponLayer_cashrec_amt")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(cashrec_n_amt))[" + document.getElementsByName("couponLayer_cashrec_n_amt")[i].value									
									+"]");
									*/
								
									document.getElementsByName("couponLayer_coupon_fg")[0].value          = c_coupon_fg;
									document.getElementsByName("couponLayer_coupon_cd")[0].value          = c_coupon_cd;
									document.getElementsByName("couponLayer_coupon_no")[0].value          = c_coupon_no;
									document.getElementsByName("couponLayer_face_amt")[0].value           = comma(c_face_amt);
									document.getElementsByName("couponLayer_cashrec_yn")[0].value         = c_cashrec_yn;
									document.getElementsByName("couponLayer_vat_cal_ext_rate")[0].value   = c_vat_cal_ext_rate;
									document.getElementsByName("couponLayer_vat_cal_rate")[0].value       = c_vat_cal_rate;
									document.getElementsByName("couponLayer_cp_chage_amt")[0].value       = comma(c_cp_chage_amt);
									document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[0].value = comma(c_cp_chage_apy_y_amt);
									document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[0].value = comma(c_cp_chage_apy_n_amt);
									document.getElementsByName("couponLayer_cashrec_amt")[0].value        = comma(c_cashrec_amt);
									document.getElementsByName("couponLayer_cashrec_n_amt")[0].value      = comma(c_cashrec_n_amt);
									
									$("#couponLayer_coupon_fg"+(i+1)).val(c_coupon_fg);
									$("#couponLayer_coupon_cd"+(i+1)).val(c_coupon_cd);
									$("#couponLayer_coupon_no"+(i+1)).val(c_coupon_no);
									$("#couponLayer_face_amt"+(i+1)).val(comma(c_face_amt));
									$("#couponLayer_cashrec_yn"+(i+1)).val(c_cashrec_yn);
									$("#couponLayer_vat_cal_ext_rate"+(i+1)).val(c_vat_cal_ext_rate);
									$("#couponLayer_vat_cal_rate"+(i+1)).val(c_vat_cal_rate);
									$("#couponLayer_cp_chage_amt"+(i+1)).val(comma(c_cp_chage_amt));
									$("#couponLayer_cp_chage_apy_y_amt"+(i+1)).val(comma(c_cp_chage_apy_y_amt));
									$("#couponLayer_cp_chage_apy_n_amt"+(i+1)).val(comma(c_cp_chage_apy_n_amt));
									$("#couponLayer_cashrec_amt"+(i+1)).val(comma(c_cashrec_amt));
									$("#couponLayer_cashrec_n_amt"+(i+1)).val(comma(c_cashrec_n_amt));
									...
									+ "] gridCoupon.valueMatrix(1, gridCoupon.colRef(cp_chage_apy_y_amt))[" + document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[0].value
									+ "] gridCoupon.valueMatrix(1, gridCoupon.colRef(cp_chage_apy_n_amt))[" + document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[0].value
									+ "] gridCoupon.valueMatrix(1, gridCoupon.colRef(cashrec_amt))[" + document.getElementsByName("couponLayer_cashrec_amt")[0].value
									+ "] gridCoupon.valueMatrix(1, gridCoupon.colRef(cashrec_n_amt))[" + document.getElementsByName("couponLayer_cashrec_n_amt")[0].value									
									+"]");
									*/
							...
							else
							{
								...
								if(i == 0 && faceAmt >= 0 )
								{
									maxChageApyAmt = resultAmt;
									document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value = 0;
									document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value = comma(faceAmt);
								}
								...
								if( (i != 0 && faceAmt >= 0 ) && (maxChageApyAmt == 0 || maxChageApyAmt > resultAmt) )	
								{
									...
									*/	
									...
									document.getElementsByName("couponLayer_coupon_cd")[i].value         = document.getElementsByName("couponLayer_coupon_cd")[0].value;
									document.getElementsByName("couponLayer_coupon_no")[i].value          = document.getElementsByName("couponLayer_coupon_no")[0].value;
									document.getElementsByName("couponLayer_face_amt")[i].value           = document.getElementsByName("couponLayer_face_amt")[0].value.replace(/,/gi, "");
									document.getElementsByName("couponLayer_cashrec_yn")[i].value         = document.getElementsByName("couponLayer_cashrec_yn")[0].value;
									document.getElementsByName("couponLayer_vat_cal_ext_rate")[i].value   = document.getElementsByName("couponLayer_vat_cal_ext_rate")[0].value;
									document.getElementsByName("couponLayer_vat_cal_rate")[i].value       = document.getElementsByName("couponLayer_vat_cal_rate")[0].value.replace(/,/gi, "");
									document.getElementsByName("couponLayer_cp_chage_amt")[i].value       = document.getElementsByName("couponLayer_cp_chage_amt")[0].value.replace(/,/gi, "");
									document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[i].value = 0;
									document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[i].value = 0;
									document.getElementsByName("couponLayer_cashrec_amt")[i].value        = document.getElementsByName("couponLayer_cashrec_amt")[0].value.replace(/,/gi, "");
									document.getElementsByName("couponLayer_cashrec_n_amt")[i].value      = document.getElementsByName("couponLayer_cashrec_n_amt")[0].value.replace(/,/gi, "");

									/*
									alert("[거스름현금영수증100 2]gridCoupon.valueMatrix(i, gridCoupon.colRef(coupon_fg))[" + document.getElementsByName("couponLayer_coupon_fg")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(coupon_cd))[" + document.getElementsByName("couponLayer_coupon_cd")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(coupon_no))[" + document.getElementsByName("couponLayer_coupon_no")[i].value 
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(face_amt))[" + document.getElementsByName("couponLayer_face_amt")[i].value
									+ "] gridCoupon.valueMatrix(i, gridCoupon.colRef(cashrec_yn))[" + document.getElementsByName("couponLayer_cashrec_yn")[i].value
			...
			if(document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[0].value != 0)
			{
				//alert("[거스름반영 전]cashrec_n_amt[" + document.getElementsByName("couponLayer_cashrec_n_amt")[0].value
				//+ "] cashrec_amt[" + document.getElementsByName("couponLayer_cashrec_amt")[0].value + "]");
					
				document.getElementsByName("couponLayer_cashrec_n_amt")[0].value = document.getElementsByName("couponLayer_cp_chage_apy_n_amt")[0].value;
				document.getElementsByName("couponLayer_cashrec_amt")[0].value = document.getElementsByName("couponLayer_cp_chage_apy_y_amt")[0].value;

				//alert("[거스름반영 후]cashrec_n_amt[" + document.getElementsByName("couponLayer_cashrec_n_amt")[0].value
				//	+ "] cashrec_amt[" + document.getElementsByName("couponLayer_cashrec_amt")[0].value + "]");								
			}
			
			for(var i = 0; i < coupon_cnt; i++)
			{
				cashrecNCoupon += nullCheck(document.getElementsByName("couponLayer_cashrec_n_amt")[i].value);
				cashrecYCoupon += nullCheck(document.getElementsByName("couponLayer_cashrec_amt")[i].value);
			}
		}					
		...
		if(couponAmt != null && couponAmt != '') 
		{
			...
			$("#coupon_amt").val(comma(coupon_amt));
		}
	}		
	...
	$.ajax({
		...
		success : function(data) 
		{
			...
			if(result.isSuc == "success")
			{
				...
				$("#mcouponLayer_remain_amt").val(comma(mcoupon_remain_amt));
				mcoupon_search_cd = 1;
			}
			else
			{
...
function mcoupon_approveCheck()
{
	...
	var sale_amt_i 			= Number($("#mcouponLayer_use_amt").val().replace(/,/gi, ""));
	...
	if( confirm(comma(sale_amt_i) + "원을 결제하시겠습니까?" ) ) {
		...
		$.ajax({
			...
			success : function(data) 
			{
				...
				if(result.isSuc == "success")
				{
					...
					mcoupon_amt = Number($("#mcouponLayer_use_amt").val().replace(/,/gi, ""));
					...
					$("#mcouponLayer_approve_amt").val(comma(mcoupon_approve_amt));
					$("#mcouponLayer_approve_amt").val(comma(mcoupon_approve_amt));
					$("#mcoupon_amt").val(comma(mcoupon_approve_amt));
					$("#mcouponLayer_approve_no").val(mcoupon_approve_no);
					$("#mc_approve_no").val(mcoupon_approve_no);
					$("#mcouponLayer_coupon_remain_amt").val(comma(mcoupon_rmn_amt));
				}
				else
				{
					alert(result.msg);
...
function AKmem_getPoint(act)
{
	akmem_card_no = "";
	if(act == "card")
	{
		akmem_card_no = $("#akmem_card_no").val();
	}
	else if(act == "basic")
	{
		akmem_card_no = $("#card_no").val();
	}
	
	if(akmem_card_no != ""){
		
		if(akmem_card_no.indexOf("*") > -1)
		{
			...
			$.ajax({
				type : "POST", 
				url : "/member/lect/GetAkmemCustInfoDecode",
				dataType : "text",
				async:false,
				cache : false,
				data : 
				...
				success : function(data) 
				{
					...
					akmem_card_no = result.akmem_card_no;
				}
			});
		}
		...
		$.ajax({
			...
			{
				var result = JSON.parse(data);
				$("#akmem_total_point").html(comma(Number(result.AKmem_total_point))+" <span>원</span>");
				akmem_card_status = result.AKmem_Resp_No;
...
//				$("#akmem_card_name").val(result.AKmem_CustName);
//				$("#akmem_total_point").html(comma(Number(result.AKmem_total_point))+" <span>원</span>");
...
//                if(nullChk(result.AKmem_CardNo) != "")
//                {
...
function AKmem_Header(pID,pType){
    ...
    var ls_inst_cd  ;
        ...
        if( ls_store == "04" )  {  ls_inst_cd   =   "1021"; }
...
function cardSelectFg(val){
	var flag = $("#normal_type").val();
	cancel_card_amt = 0;
...
function cardController(gubun){
	...
	if(gubun == "card") {
		...
		sendData = "0"					//거래 구분자	AN(1)	“0” :  신용  , “1” : 교통 , “2” : 신용 + 멤버쉽
			   +"0"					//거래 종류	AN(1)	“0” :  구매  , “1” : 취소
			   +getTimeStamp()		//거래 일시	AN(14)	YYYYMMDDHHMMSS
			   +f_setfill(card_amt,9,'L')				//거래 금액	AN(9)	Right Justfy
			   +f_setfill_zero(pos_no,8,'L')  			//단말기 ID	AN(8)	"단말기 ID(TID) :“0”으로 Padding8자리가 넘는 경우 8자리까지만 사용"
			   +"2"					//EMV PIN 설정	AN(1)	"“1” : PIN 입력,“2” : PIN 입력 안 함 (ED-946 은 사용안함)“3” : 은련만 PIN 입력"
			   +"4"				    //Card Input 설정	AN(1)	"“1” : MS/IC, “2” : Only MS, “3” : Only IC( 기능 Disable )“4”:KEYIN : 서명패드 에서 지원"
			   +"2"					//"EMV MS거래 허용 여부"	AN(1)	"“1” : MS 거래허용, “2” : MS 거래불가( 기능 Disable )"
			   +f_setfill("21",16,'R')				//SEED DATA	AN(16)	RANDOM DATA		
			   +"05"						//요청 갯수	AN(2)	00 ~ 99 : “00” 일 경우 스크래치 Data 전송 후 대기 값이 없는 경우 VAN 사 정보만 리딩
		       +"140000200300944541942150451842";				//카드사 BIN 	AN(6xN)	RANDOM DATA	      
			   //+"03"				//요청 갯수	AN(2)	00 ~ 99 : “00” 일 경우 스크래치 Data 전송 후 대기 값이 없는 경우 VAN 사 정보만 리딩
			   //+"140000200300944541";			
		ret = cardX.ReqCmd( 0xFB, 0x11, 0x20, sendData, sE); // 단말기 키인입력 호출
		ret = cardX.WaitCmd( 0xFB, cardX.RcvData, 1000, 1, "처리중입니다.", sE);// 단말기 키인입력 호출 결과대기
	}
...
//function akmem_use()
//{
//	var this_val = $("#point_amt").val().replace(/,/gi, "");
//	this_val = this_val.replace(/[^0-9]/g, '');
//	$("#point_amt").val(comma(this_val));
//	
//	var pointAmt = 0;
...
//	
//	point_amt = $("#point_amt").val().replace(/,/gi, "");
//	//재료비체크
//	if(total_food_amt > 0) {
...
//		akmemUseMaxPoint = Number(Number(Number(akmem_total_point)/akmemUseMinPoint)*akmemUseMinPoint);
//	}
//
...
//		{
//			var result = JSON.parse(data);
//			akmem_use_sApprovNo = result.sApprovNo;
//			if(akmem_use_sApprovNo != '00')
//			{
//				alert("[포인트조회 오류!]"+result.sMessage);
...
function QRChipData()
{
	if($("input[name=payment_type]:checked").val() == "simple")
	{
		console.log("1111111111111111");
		var QrData = $("#ic_card_no").val()
		...
		try {
			ret = cardX2.GetQRChipData(QrData, AddData, sR, 0);	// BCQR 데이터를 dll 호출하여 리턴값을 받아온다.
			sR = cardX2.Data;														// 받아온 데이터
...
//	search_fg.disabled = false;
	...
	console.log("zzzzzzzzzzzzz22222222");
	var ls_ret_str = f_card_gubun_nm("new", qr_card_no_real);	// 카드정보를 가져 온다.
	console.log("zzzzzzzzzzzzz333333333 : "+ls_ret_str); 
	var ls_card_fg			    = ls_ret_str.substring(0, 1);
	ic_card_co_origin_seq	= ls_ret_str.substring(1, 4);
	ic_card_sain_fg		    = ls_ret_str.substring(4, 5);
	ic_card_nm				= ls_ret_str.substring(5);
	console.log("zzzzzzzzzzzzz44444444");
	is_wcc = "I";		// IC카드 거래로 인식 하게끔 -> 승인조회시  카드에 따라 Q 나 q 로 변경
	if($("#simple_type").val() == "bc")
	{
		ls_card_fg = "Q"; // KICC 로 van 청구 해야 함.
	}
	else if($("#simple_type").val() == "naver")
	{
		ls_card_fg = "N"; // KICC 로 van 청구 해야 함.
	}

	$("#ic_card_no").val(qr_card_no);
	$("#ic_card_nm").val(ic_card_nm);
	ic_card_no = qr_card_no_real;
	ic_card_data_fg = ls_card_fg;
	ic_card_encCardNo_send_str = qr_card_no_real;
...
{
//	if(act == "send" && $("#ic_card_no").val() == "")
//	{
//		alert("카드를 등록해주세요.");
//		return;
//	}
		
	var isCancling = false;
	cancel_card_amt = 0;
	var isTid = false;
	$("[name='pere_idx']").each(function() 
	{
	...
	if(isCancling && act == "send")
	{
		alert("결제 취소 동작 중 승인번호를 조회할 수 없습니다.");
		return;
	}
	
	if(ic_card_no == "" && cancel_card_amt > 0 && act != 'super_cancel' && !isTid)
	{
		alert("취소처리할 카드를 등록해주세요.");
		return;
	}

	if(cancel_card_amt == 0 && (act == 'cancel' || act == 'super_cancel'))
	{
		signText = "1";
		posCancel(act);
		return;
	...
	else if(act == 'cancel' || act == 'super_cancel')
	{
		cardAmt = cancel_card_amt;
	}
	
	console.log("ls_halbu_no" + ls_halbu_no);
	...
	if(ls_halbu_no == "00") {
		...
		console.log("할부 00");
		console.log("ls_card_corp_cd" + ls_card_corp_cd);
...
//		}
		...
		}else if(pos_no == '070020') {
			signText = "1";
			alert("관리자 모드입니다. 서명하지않습니다.");
			if(act == 'send'){approveCheck();}
			else{posCancel(act);}
		}else{       
			signView();
...
function closeSignView()
{
	...
	console.log("ic_card_sign : "+ic_card_sign);
	if(signAct == "send")
	{
		console.log("signAct : "+signAct);
		approveCheck();
	}
	else 
	{
...
//						+ f_setfill(ls_royalty,203,"R")  + f_setfill(ls_resi_no,13,"R");
	...
	if(bc_qr_value != null && bc_qr_value != ""  ) {
		console.log("bc_qr_value : "+bc_qr_value);
		// BC QR 전문 생성 시작
		...
		var qr_emv_data = bc_qr_data.substring(47, bc_qr_value.length - 46);
		var qr_gubun      = bc_qr_data.substr(3, 1) ; // (L, C)
		var check_bill_fg = "";
		
		//네이버페이여도 디비에는 N으로 저장하지만, 전문에는 Q로 보내야 승인이된다.
...
//	}
	...
	+ eval(nullCheck(point_amt))   //2019.03.11 ljs : 사용마일리지(akmem_point_amt) 추가
	- eval(nullCheck(final_pay)) 
	- eval(nullCheck($("#change").val().replace(/,/gi, "")));
	...
	if(Number(point_amt) > 0 && Number(point_amt) % 10 > 0)
	{
		alert("마일리지는 10원단위로 사용 가능합니다.");
		return;
	}
	
	if(Number(point_amt) > 0 && pos_no != '070020')
	{
		var keyVl = keyInControl_akmem();
		var aKmemPswd = akmem_pswd;
		var keyTryYn = "N";
    ...
    for(var i = 0; i < coupon_cnt; i++)
	{
    	if( ( document.getElementsByName("couponLayer_coupon_no")[i].value == null
  			  || document.getElementsByName("couponLayer_coupon_no")[i].value == "" )
  			 && ( document.getElementsByName("couponLayer_face_amt")[i].value != null
  			     && document.getElementsByName("couponLayer_face_amt")[i].value != ""  )
  		   ){
    ...
    if(cash_amt > 0 || eval(coupon_amt) > 0 || eval(mcoupon_amt) > 0)
	{
    	//현금승인번호가 없으면
		if(isNull(cash_approve_no)) 
        {
			...
			if(eval(cash_amt) - nullCheck(change) > 0)
			{
			//2019.04.26  ljs 현금승인금액체크 0원인 경우 처리 END						

				//현금금액이 5000원이상이면
				//현금금액이 0원 이상 이면
				if(Number($("#cash_amt").val().replace(/,/gi, "")) >0) {		// 현금
					...
					//	return;
					//}
				} else if(Number($("#coupon_amt").val().replace(/,/gi, "")) >= 5000) {		// 지류상품권
				//if(confirmMsg("5000원 이상의 현금지불은 현금영수증을 발급받아야 합니다.\n현금영수증 페이지로 이동하시겠습니까?")) {
					openCashLayer();
					return;
				//} else {
				//	return;
				//}
				} else if(Number($("#mcoupon_amt").val().replace(/,/gi, "")) > 0) {		// 모바일상품권			2017.08.24
					openCashLayer();
					return;
				}
			}
  ...
  //AK기프트(555),홈플러스(666)카드 현금영수증 발급 추가(2012.01.12) ★★★★★★★★★★★★★★★------------
	...
	//카드금액 확인
	if(card_amt != $("#card_amt").val().replace(/,/gi, "")) {
	...
	*/
	...
	for(var z = 0; z < subject_arr.split("|").length-1; z++)
	{
		child_no1_arr += child1_no.get(subject_arr.split("|")[z])+"|";
		child_no2_arr += child2_no.get(subject_arr.split("|")[z])+"|";
		child_no3_arr += child3_no.get(subject_arr.split("|")[z])+"|";
		encd_no1_arr += encd1_no.get(subject_arr.split("|")[z])+"|";
		encd_no2_arr += encd2_no.get(subject_arr.split("|")[z])+"|";
		encd_amt1_arr += encd1_amt.get(subject_arr.split("|")[z])+"|";
		encd_amt2_arr += encd2_amt.get(subject_arr.split("|")[z])+"|";
		part_regis_fee_arr += part_regis_fee.get(subject_arr.split("|")[z])+"|";
		part_food_amt_arr += part_food_amt.get(subject_arr.split("|")[z])+"|";
	}
	var credit_yn = "N";
	if($("input:checkbox[name='credit_yn']").is(":checked"))
	{
		credit_yn = "Y";
	}
	$.ajax({
		...
		data : 
		{
			...
			akmem_point_amt : point_amt,
			akmem_card_no : akmem_card_no,
			akmem_card_status : akmem_card_status,
			coupon_amt : coupon_amt,
			md : md,
			op : op,
			goods : goods,
			subject_arr : subject_arr,
			part_subject_arr : part_subject_arr,
			child_no1_arr : child_no1_arr,
			child_no2_arr : child_no2_arr,
			child_no3_arr : child_no3_arr,
			encd_no1_arr : encd_no1_arr,
			encd_no2_arr : encd_no2_arr,
			encd_amt1_arr : encd_amt1_arr,
			encd_amt2_arr : encd_amt2_arr,
			part_regis_fee_arr : part_regis_fee_arr,
			part_food_amt_arr : part_food_amt_arr,
			...
			cp_chage_amt_arr : cp_chage_amt_arr,
			...
			change : $("#change").val().replace(/,/gi, "") //거스름돈
		...
		success : function(data) 
		{
			...
			if(result.isSuc == "success")
    		{
				akmem_cust_no = result.akmem_cust_no;
				akmem_save_approve_no = nullChk(result.akmem_save_approve_no); //마일리지 적립승인번호
				akmem_sApprovNo = nullChk(result.akmem_sApprovNo);//마일리지 사용승인번호
				akmem_recpt_point = result.akmem_recpt_point;//마일리지 사용승인번호
				akmem_save_approve_no_point = result.akmem_save_approve_no_point; //누적마일리지
    			alert(result.msg);
	...
	//			if(ic_card_month == '00')
	//			{
	    			console.log("c_print_stat : "+c_print_stat);
	    			if(c_print_stat == "ON")
	    			{
...
//	    				{
	    					if(posInit()) {
	    						posPrint('C');
	    						if(printer.DeviceEnabled && pos_no != '070020') {
...
//	    				}
	...
	//			}
	    			...
	    			if(pos_no != '070020')
	    			{
	    				location.href="/member/lect/view?data="+$("#cus_no").val()+"_00_000_000000";
...
function AKpoint_Calc(){
	...
	for(var i = 0; i < coupon_cnt; i++)
	{
		pAkCoupon += Number(document.getElementsByName("couponLayer_face_amt")[i].value);
	}
...
function nullCheck(str) {
	if(str == "" || str == null) {
		...
		return eval(str.toString().replace(/,/gi, ""));
	}
}
function posInit() {
	console.log("posInit");
	...
	{
		if(printer.DeviceEnabled ) {
			console.log("posInit1");
			printer.ReleaseDevice();
			printer.close();
		} 
		
		if (printer.Open("IPOS_PRINTER") != 0) {
			console.log("posInit2");
			posInit2();
		} else {
			console.log("posInit2.5");
			if (printer.ClaimDevice("1000") != 0 ) {
				console.log("posInit3");
				posInit2();
			} else {
				return true;
			}
...
//	한번은 되고 한번은 안되고 반복되어서 실패한경우 한번 더 호출
}//posInit()
function posInit2() {
	console.log("posInit4");
	if(printer.DeviceEnabled ) {
		console.log("posInit5");
		printer.ReleaseDevice();
		printer.close();
	} 
	
	if (printer.Open("IPOS_PRINTER") != 0) {
		console.log("posInit6");
...
//		var ret;
//
//	    console.log(cardX.ReqReset()+"aaaa");
//	    
//		if(setCardReaderOpen())
//		{
//			danMaldataParse(cardX.RcvData);
//			CardReaderIntegrity();		// 무결성 체크
//			Insert_Integrity();			// DB저장
...
function f_card_gubun_nm(gubun, card_no) {
    ...
    if(li_row > 0) {
    	...
    	console.log("aaaa2222 : "+card_no);
        var ls_card_no = null;
        ls_card_no = card_no.substring(0, 6);
        console.log("aaaa2222 : "+ls_card_no);
        ...
        for(li_row = 6; li_row > 0; li_row--) {
            ...
            console.log("aaaa4444 : "+ic_card_co_origin_seq);
...
function posPrint(flag) {
	...
	ls_data += ls_escape + ls_doublewide + "  [ 매 출 등 록 ]\n"
	...
	//에누리
	ls_enuri_amt 		= $("#encd_pay").val().replace(/,/gi, "");
	...
	if(coupon_fg_arr != "") 
	    ...
	    for(var i = 0; i < coupon_cnt; i++)
		{
	    	if(document.getElementsByName("couponLayer_coupon_fg")[i].value == "0" 
				   || document.getElementsByName("couponLayer_coupon_fg")[i].value == "7") 
			{
				my_coupon_count += 1;
				my_Face_Amt +=  parseInt(document.getElementsByName("couponLayer_face_amt")[i].value.replace(/,/gi, ""));
				
				if(my_coupon_no == "" || my_coupon_no == null )
				{
					my_coupon_no = document.getElementsByName("couponLayer_coupon_no")[i].value; 
				}
            } 
             else
			{
                other_coupon_count += 1;
				other_Face_Amt +=  parseInt(document.getElementsByName("couponLayer_face_amt")[i].value.replace(/,/gi, ""));
				
				if(other_coupon_no == "" || other_coupon_no == null )
				{
					other_coupon_no = document.getElementsByName("couponLayer_coupon_no")[i].value; 
				}
			} 
		}
	...
	if(nullChk(akmem_save_approve_no) != "" || nullChk(akmem_sApprovNo) != "")
	{
		if(nullChk(akmem_sApprovNo) == '')
		{
			akmem_sApprovNo = akmem_save_approve_no;
		}
		...
		ls_data += "AK멤버스 승인번호 :  " + akmem_sApprovNo +'\r\n' //akmem_save_approve_no
		ls_data += "AK멤버스 적립 마일리지 : " + f_setfill(comma(Number(akmem_recpt_point)), 13,'L') +'점\r\n'
		//2019.03.11 ljs 사용마일리지 추가
		ls_data += "AK멤버스 사용 마일리지 : "  + f_setfill(comma(point_amt), 13,'L') +'점\r\n'  //2019.03.11 ljs  사용마일리지 추가
		ls_data += "AK멤버스 누적 마일리지 : "  + f_setfill(comma(Number(akmem_save_approve_no_point)), 13,'L') +'점\r\n'
...
//		}
	...
	if(ls_aprv_no != null && ls_aprv_no != "") {
		...
		var tmp_cash_amt = 0;
		if ((ic_card_co_origin_seq == "555") || (ic_card_co_origin_seq == "666")) {
			tmp_cash_amt = eval(nullCheck(cash_amt)) 
						+ eval(nullCheck(coupon_amt))	 
						+ eval(nullCheck(mcoupon_amt))    
						+ eval(nullCheck(card_amt))
						- eval(nullCheck(cashrec_n_coupon)) ;	//2019.03.11 ljs : 증정상품권 현금영수증제외 금액 추가 
		...
		ls_data += '승인번호:' + ls_aprv_no + '  인정금액:' + comma(eval(tmp_cash_amt) - nullCheck(ls_change))+ '원\r\n'
		ls_data += '----------------------------------------\r\n'
...
//			if(act == "super_cancel"){
//				if(result.CARD_FG != "Q"){
//					if(!(daydiff(result.SALE_YMD) < 90 && nullChk(result.CARD_NO) != "" )
//						|| (result.CARD_TYPE == "Check/선불카드" || getNow() == result.SALE_YMD || result.CARD_CORP_NO == "555" || result.CARD_CORP_NO == "666") || nullChk(result.TID) != ""){
...
//			}else{ //매출취소일때		
				...
				if( card_data_fg == "Q" || card_data_fg == "q" || card_data_fg == "N" || card_data_fg == "n") {	//bcqr 일 시 에는 카드센싱 필요 x
...
//				
				...
				if(ls_approve_no != null && ls_approve_no != '' && act == "cancel") {
					// 당일매출 당일취소이면 승인취소	
...
//						autoSign('cancel');
						...
						if(tid == null || tid == ""){
							...
							$.ajax({
								...
								{
									isSuc = true;
									if(data != "SUCCESS" && act != "super_cancel"){
...
//				model.setValue("/root/req/total_show_amt",	getGridColumnNameValue(gridMain, gridMain.row, "trade_all_amt")); 
				...
				$.ajax({
					...
					data : 
					{
						...
						akmem_approve_no : result.AKMEM_APPROVE_NO,
					...
					success : function(data) 
					{
						...
						var result_gift = JSON.parse(data);
						if(result_gift.isGift == "Y")
						{
							alert(result_gift.gift_msg.replace(/<br>/gi, "\n"));
						}
						
						alert("매출취소가 완료되었습니다.");
						var tmp_pos_no = cancle_pos_no;
						if(result.POS_NO == "070013" || result.POS_NO == "070014")
						{
							tmp_pos_no = result.POS_NO
						}
						cancelPrint($("#selBranch").val(), result.PERIOD, tmp_pos_no, cancel_new_recpt_no);
						if(pos_no != '070020')
		    			{
							location.href="/member/lect/view?data="+$("#cus_no").val()+"_00_000_000000";
							dataReset(); //변수 초기화
	...
	$.ajax({
		...
		success : function(data) 
		{
			if(data != "")
			{
				...
				if(pos_open_yn == null || pos_open_yn == "" || pos_open_yn == "N") {
					if(pos_no != "070020")
					{
						alert("준비금 등록을 먼저 하십시요!");
						location.href="/trms/trms/trms_write";
						return false;
					}
				}
				...
				if(pos_end_yn == "Y") {
					if(pos_no != "070020")
					{
						...
						location.href="/trms/trms/close_write";
						return false;
					}
				}
			}
...
//	var ls_pos_no 	= model.getValue("/root/req/pos_no");
	...
	}else if( ls_cardvan_fg == "20") {
		//"CB013S" IC-SMARTRO 타사 카드 취소 조회
		//"CB013R" IC-SMARTRO 타사 카드 취소 응답
		ls_send_programID = "CB013S";
	}
...
//					+ f_setfill(ls_filler,2,"R") +  f_setfill(ls_working,16,"R") + f_setfill(ls_royalty,203,"R")  + f_setfill(ls_resi_no,13,"R");	
	...
	if(bc_qr_data == null || bc_qr_data == ""  ) {
		...
		if(card_data_fg == "Q" || card_data_fg == "q" || card_data_fg == "N" || card_data_fg == "n") {		//bc QR 문 만들어야 함.  cmc
			// BC QR 전문 생성 시작
	...
	$.ajax({
		...
		success : function(data) 
		{
			...
			if(ls_msg.indexOf("취소거절이미매입된거래") > -1 || ls_msg.indexOf("[0058]") > -1) //20210911 정기영 0058 정상처리
				ls_msg = "정상거래";
...
{
//	posInit();
//	if(nullChk(printer.DeviceEnabled) == "")
//	{
//		alert("프린터기를 확인해주세요.");
//		return;
//	}
...
//				if(ls_card_amt > 0 || ls_mc_card_amt > 0 || ls_cash_amt > 0) {
				if(c_print_stat == "ON")
    			{
//					if(confirm("고객영수증을 출력하시겠습니까?"))
//					{
...
function cancelPrint(store, period, pos_no, recpt_no)
{
//	posInit();
//	if(nullChk(printer.DeviceEnabled) == "")
//	{
...
//	}
	if(posInit()) {
		$.ajax({
			...
			{
				var result = JSON.parse(data);
				//encodeURI(JSON.stringify(result[i]))
				//전역변수 셋팅
				result[0].IN_TYPE = "취소";
				pos_detail(encodeURI(JSON.stringify(result[0])));
				var ls_card_amt 	= result[0].CARD_AMT;
				ls_card_amt			= ls_card_amt == null || ls_card_amt == "" ? 0 : parseInt(ls_card_amt); 
				var ls_cash_amt 	= result[0].CASH_AMT;
				ls_cash_amt			= ls_cash_amt == null || ls_cash_amt == "" ? 0 : parseInt(ls_cash_amt);
				var ls_mc_card_amt 	= result[0].MC_CARD_AMT;
				ls_mc_card_amt		= ls_mc_card_amt == null || ls_mc_card_amt == "" ? 0 : parseInt(ls_mc_card_amt);
				var ls_e_sign 		= result[0].E_SIGN;
				//2019.04.29 ljs 취소된 후 영수증출력시 마일리지 정보 조회
				var ls_akmem_approve_no = result[0].AKMEM_APPROVE_NO;
//				if(ls_card_amt > 0 || ls_mc_card_amt > 0 || ls_cash_amt > 0) {
				if(c_print_stat == "ON")
    			{
//					if(confirm("고객영수증을 출력하시겠습니까?"))
//					{
						pos_header('C', encodeURI(JSON.stringify(result[0])));	// 고객용
						pos_footer('C', encodeURI(JSON.stringify(result[0])));	// 카드
						posPrintPrev();
						
						if(ls_card_amt > 0 && ls_e_sign != "1") {
							pos_header('S', encodeURI(JSON.stringify(result[0])));	// 카드전표
							pos_footer('S', encodeURI(JSON.stringify(result[0])));	// Card 수거용
							posPrintPrev();
						}
//					}
    			}
...
//					if(confirm("가맹영수증을 출력하시겠습니까?"))
//					{
						pos_header('A', encodeURI(JSON.stringify(result[0])));
						pos_footer('A', encodeURI(JSON.stringify(result[0])));
						posPrintPrev();
//					}
    			}
//				}
...
function pos_header(flag, json) {
	...
	ls_coupon_amt       = result.COUPON_AMT; 
...
//분당점 에이알디홀딩스(주) -> AK S&D(주) 변경(2012.06.26)
		...
		ls_header_data += "129-85-42346  김 재 천\n"
...
//		ls_header_data += "항상 최고로 모시겠습니다.\r\n\r\n"	
		...
		ls_header_data += "124-81-28579  김 재 천\n"
...
//		ls_header_data += "항상 최고로 모시겠습니다.\r\n\r\n"	
		...
		ls_header_data += "125-81-24085  김 재 천\n"
		...
		ls_header_data += "224-85-23362  김 재 천\n"
...
function pos_footer(flag, json) {
	...
	var result = JSON.parse(decodeURI(json));
	console.log("영수증재발행 푸터 : "+decodeURI(json));
	var sub_tot_fee = 0;
	$.ajax({
		type : "POST", 
		url : "/member/lect/getCancelSubject",
	...
	ls_repay_amt        = result.REPAY_AMT;
	ls_repay_amt		= ls_repay_amt == null || ls_repay_amt == "" ? 0 : parseInt(ls_repay_amt);
	ls_change			= result.CHANGE;
	ls_change			= ls_change == null || ls_change == "" ? 0 : parseInt(ls_change);
//	ls_change           = ls_repay_amt + ls_change;
	...
	var ls_card_amt         =	result.CARD_AMT;	
	var ls_coupon_amt       =   result.COUPON_AMT;
	var ls_approve_no 		=	result.APPROVE_NO;
...
// 발행사 표기 추가(12.11.20)
			...
			//cmc123456
			if(ls_bc_qr_value == "Q" || ls_bc_qr_value == "q" || ls_bc_qr_value == "N" || ls_bc_qr_value == "n" ) {
...
//여전법 이전			ls_footer_data += '카드번호:'+ ls_card_no.substring(0, 4) + "-" + ls_card_no.substring(4, 8) + "-****-" + ls_card_no.substring(12)  +  "       **/**" + '\r\n';
		...
		if(ls_coupon_amt > 0) {
			$.ajax({
				...
				success : function(data) 
				{
					...
					var coupon_cnt = result_this.length;
					var sum_cashrec_n_amt = 0;
				    
				    for(var i = 0; i < coupon_cnt; i++)
					{
				    	if(result_this[i].COUPON_FG == "0" || result_this[i].COUPON_FG == "7") 
						{
							my_coupon_count += 1;
							my_Face_Amt +=  parseInt(result_this[i].FACE_AMT);
							sum_cashrec_n_amt +=  parseInt(result_this[i].CPN_ENURI_AMT);
							if(my_coupon_no ==  null || my_coupon_no == "" || my_coupon_no == undefined)
							{
								my_coupon_no = result_this[i].COUPON_NO;
							}
			            } 
			             else
						{
			                other_coupon_count += 1;
							other_Face_Amt +=  parseInt(result_this[i].FACE_AMT);
							sum_cashrec_n_amt +=  parseInt(result_this[i].CPN_ENURI_AMT);
							
							if(other_coupon_no == "" || other_coupon_no == null )
							{
								other_coupon_no = result_this[i].COUPON_NO;
							}
						} 
					}
					//자사상품권표기
					if(my_coupon_no != "" && my_coupon_no != null )
					{
						ls_footer_data += "자사상품권" + f_setfill("",5,'R');
						ls_footer_data += f_setfill(comma(my_coupon_count.toString()), 10, 'L') + " 매";
						ls_footer_data += f_setfill(comma(my_Face_Amt.toString()),10,'L') + '원\r\n';
						ls_footer_data += '상품권 번호 : ' + my_coupon_no + '\r\n';
					}
					//타사상품권표기
					if(other_coupon_no != "" && other_coupon_no != null )
					{
						ls_footer_data += "타사상품권" + f_setfill("",5,'R');
						ls_footer_data += f_setfill(comma(other_coupon_count.toString()), 10, 'L') + " 매";
						ls_footer_data += f_setfill(comma(other_Face_Amt.toString()),10,'L') + '원\r\n';
						ls_footer_data += '상품권 번호 : ' + other_coupon_no + '\r\n';							
					}
					if(sum_cashrec_n_amt > 0)
					{
						ls_footer_data += '----------------------------------------\r\n' ;
						ls_footer_data += "   (자사증정-현금영수증제외)" + f_setfill(comma(sum_cashrec_n_amt.toString()), 10, 'L') + "원\r\n";
					}
					return;
				}
			});
		...
		//AKmembers 차감 기록 출력
		console.log("zzz"+cancle_proc_point_approve_no);
		console.log("zzz"+nullChk(result.AKMEM_APPROVE_NO));
		if(cancle_proc_point_approve_no != "" || nullChk(result.AKMEM_APPROVE_NO) != "")  //2019.04.29 ljs 취소후 영수증출력시 마일리지 정보가 있는 경우
		{
//			if(result.IN_TYPE == "취소")
//			{
...
//			}
		...
		if(ls_aprv_no != null && ls_aprv_no != ""&& ls_aprv_no != "N") {
			...
			if ((ls_card_fg == "555") || (ls_card_fg == "666")) {
				...
				if(result.IN_TYPE == "취소")
				{
					ls_footer_data += '승인번호:' + ls_aprv_no + '  인정금액:' + comma('-'+(eval(ls_cash_amt) + eval(ls_mc_card_amt) + eval(ls_coupon_amt) - eval(ls_cashrec_ext_amt)))+ '원\r\n';
				}
				else
				{
					ls_footer_data += '승인번호:' + ls_aprv_no + '  인정금액:' + comma((eval(ls_cash_amt) + eval(ls_mc_card_amt) + eval(ls_coupon_amt) - eval(ls_cashrec_ext_amt)))+ '원\r\n';
...
//----------------------------------------------------------------------
				...
				success : function(data) 
				{
				    ...
				    for(var i = 0; i < coupon_cnt; i++)
					{
			             ...
			             else
						{
							...
							if(other_coupon_no ==  null || other_coupon_no == "" || other_coupon_no == undefined)
							{
								other_coupon_no = result_this[i].COUPON_NO;
							}
						} 
		...
		//AKmembers 차감 기록 출력
		console.log("zzzㅋ"+cancle_proc_point_approve_no);
		console.log("zzzㅋ"+nullChk(result.AKMEM_APPROVE_NO));
		if(cancle_proc_point_approve_no != "" || nullChk(result.AKMEM_APPROVE_NO) != "")  //2019.04.29 ljs 취소후 영수증출력시 마일리지 정보가 있는 경우
		{
//			if(result.IN_TYPE == "취소")
//			{
...
function getPrint_AKmembers(json){
    ...
    if(nullChk(result.AKMEM_APPROVE_NO) != "")
	{
		...
		var ls_akmem_custno = result.AKMEM_CUSTNO;
		var ls_akmem_approve_no = result.AKMEM_APPROVE_NO;
		var ls_akmem_recpt_point = result.AKMEM_RECPT_POINT; 
		var ls_akmem_use_point = result.AKMEM_USE_POINT;
		...
		else
		{
			...
			data += "AK멤버스 회원번호 :  " + ls_akmem_custno +'\r\n'
			data += "AK멤버스 승인번호 :  " + ls_akmem_approve_no +'\r\n'						
			data += "AK멤버스 적립 마일리지 : " + f_setfill(comma(ls_akmem_recpt_point), 13,'L') +'점\r\n'
			data += "AK멤버스 사용 마일리지 : " + f_setfill(comma(ls_akmem_use_point), 13,'L') +'점\r\n'
			data += "AK멤버스 누적 마일리지 : "  + f_setfill(comma(ls_card_in_amt), 13,'L') +'점\r\n'
		}
	}
//	else
...
function chgCoupon(chg, idx)
{
	...
	//권종구분 선택시 액면금액에 금액 자동으로 뿌려주기
		...
		case "02" :
			$("#couponLayer_face_amt"+idx).val("2,000");
			break;
		case "03" :
			$("#couponLayer_face_amt"+idx).val("3,000");
			break;
		case "05" :
			$("#couponLayer_face_amt"+idx).val("5,000");
			break;
		case "11" :
			$("#couponLayer_face_amt"+idx).val("10,000");
			break;
		case "13" :
			$("#couponLayer_face_amt"+idx).val("30,000");
			break;
		case "15" :
			$("#couponLayer_face_amt"+idx).val("50,000");
			break;
		case "17" :
			$("#couponLayer_face_amt"+idx).val("70,000");
			break;
		case "21" :
			$("#couponLayer_face_amt"+idx).val("100,000");
			break;
		case "23" :
			$("#couponLayer_face_amt"+idx).val("300,000");
			break;
		case "25" :
			$("#couponLayer_face_amt"+idx).val("500,000");
			break;
		default :
		break;
		}
