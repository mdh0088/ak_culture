		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<td>'+cutDate(result.list[i].BMD)+'</td>';
	...
	else
	{
		...
		$.ajax({
			...
			success : function(data) 
			{
				...
				var tot_regis_fee = 0; //강사료 지급액
				if(result[0].FIX_PAY_YN == "Y")
				{
					tot_regis_fee = Number(result[0].FIX_AMT) * (Number(result[0].LECT_CNT));
					$("#prev_fix_pay_yn").val("정액");
					$("#prev_fix_data").val(comma(result[0].FIX_AMT));
				}
				if(result[0].FIX_PAY_YN == "N")
				{
					tot_regis_fee = Number(result[0].REGIS_FEE) * (Number(result[0].REGIS_NO) + Number(result[0].WEB_REGIS_NO)) * (Number(result[0].FIX_RATE) / 100);
					$("#prev_fix_pay_yn").val("정률");
					$("#prev_fix_data").val(result[0].FIX_RATE);
				}
...
function sendChange()
{
	$.ajax({
		...
		data : 
		{
			...
			period : period,
			subject_cd : subject_cd,
			prev_fix_pay_yn : $("#prev_fix_pay_yn").val(),
			prev_fix_data : $("#prev_fix_data").val(),
			next_fix_pay_yn : $("#next_fix_pay_yn").val(),
			next_fix_data : $("#next_fix_data").val(),
	...
	<div class="table-cap table">
		...
		<div class="cap-r text-right">
			<div class="table table02 table-auto float-right sel-scr">
				...
				<div>	
					<a class="btn mrg-lr6 btn03" onclick="javascript:change();">변경신청</a>				
...
<div id="change_layer" class="list-edit-wrap">
	<div class="le-cell">
		<div class="le-inner">
        	<div class="list-edit white-bg ip-edit edit-scroll">
				...
				<!-- 여기 -->
								...
								<div class="top-row">
									<div class="wid-10">
										<div class="table">
											...
											<div class="wid5-inp">
												<div class="txt text-center">변경 전</div>
											</div>
											<div class="wid5-sel">
												<div class="txt text-center">변경 후</div>
											</div>
										</div>
									</div>
								</div>
									...
									<div class="wid-10">
										<div class="table">
											...
											<div class="wid5-sel text-center ok-red">
												<select de-data="정률" id="next_fix_pay_yn" name="next_fix_pay_yn">
													<option value="N">정률</option>
													<option value="Y">정액</option>
												</select>
												<input type="text" id="" name="" class="text-center dis-no ok-red" value="정률">
											</div>
										</div>
									</div>
								</div>
								...
								<div class="top-row">
									<div class="wid-10">
										<div class="table">
											...
											<div class="wid5-inp">
												<input type="text" id="next_fix_data" name="next_fix_data" class="text-right " value="50">
											</div>
										</div>
									</div>
								</div>
								...
								<div class="top-row">
									<div class="wid-10">
										<div class="table">
											...
											<div class="wid5-inp">
												<input type="text" id="" name="" class="inputDisabled text-right " value="">
											</div>
										</div>
									</div>
								</div>
								...
								<div class="top-row">
									<div class="wid-10">
										<div class="table">
											...
											<div class="wid5-inp">
												<input type="text" id="prev_sonic_pay" name="prev_sonic_pay" class="inputDisabled text-right inptxt" value="">
											</div>
											<div class="wid5-inp">
												<input type="text" id="" name="" class="inputDisabled text-right " value="">
											</div>
								...
								<div class="top-row">
									<div class="wid-10">
										<div class="table table-area">
											...
											<div class="">
												<textarea type="text" id="change_reason" name="change_reason" class="" value="구두 협의에 따른 정률 5% 인상요청 수렴" placeholder="내용을 입력해주세요."></textarea>
											</div>
										</div>
									</div>
								</div>
					...
					<div class="btn-wr text-center">
						<a class="btn btn02 ok-btn" onclick="javascript:sendChange();">변경신청</a>
					</div>
				</div>
        	</div>
        </div>
