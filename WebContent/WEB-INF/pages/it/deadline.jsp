		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<td>'+result.list[i].PGM_NM+'</td>';
					inner += '	<td>'+cutDate(nullChk(result.list[i].REQ_START_TM))+'</td>';
					inner += '	<td>'+result.list[i].STATUS+'</td>';
					inner += '	<td>'+cutDate(nullChk(result.list[i].JOB_START_TM))+'</td>';
					inner += '	<td>'+cutDate(nullChk(result.list[i].JOB_END_TM))+'</td>';
...
function fncSubmit(act)
{
	...
	if(act == "jaemu_cancle")
	{
		$("#fncForm").attr("action", "./jaemu_dead_proc");
	}
	else
	{
		$("#fncForm").attr("action", "./dead_proc");
	}
	$("#fncForm").ajaxSubmit({
		success: function(data)
		{
...
<div class="sub-tit">
	...
	<div class="btn-right">
		<a class="btn btn01" onclick="javascript:fncSubmit('cancle');">분개 취소</a>
		<a class="btn btn01" onclick="javascript:fncSubmit('jaemu_cancle');">전송 취소</a>
...
<form id="fncForm" name="fncForm" method="POST">
	<div class="table-top">
		<div class="top-row sear-wr">
			...
			<div class="wid-2">
				<div class="table table-auto">
					<div class="sear-tit sear-tit_left">지급일자</div>
					<div>
