		...
		success : function(data) 
		{
			...
			if(result.list.length > 0)
			{
				for(var i = 0; i < result.list.length; i++)
				{
					...
					inner += '	<span class="td-350">'+result.list[i].POPUP_TITLE+'</span>';
					...
					inner += '	<span class="open"><a href="./popup_upload?seq='+result.list[i].SEQ+'"><b>수정</b></a></span>';
					inner += '</li>';
				}
			}
			else
...
function setSort()
{
	for(var i = 0; i<document.getElementsByName('seqs').length; i++)
	{
		...
		$.ajax({
			...
			url : "./sortPopup?sort="+i+"&seq="+seq,//호출 URL을 설정한다. GET방식일경우 뒤에 파라티터를 붙여서 사용해도된다.
...
function preview()
{
	...
	window.open("./preview_popup?seq="+chkList);
}
</script>
...
<div class="sub-tit">
	...
	<div class="btn-right">
		<A class="btn btn01 btn01_1" href="/web/popup_upload"><i class="material-icons">add</i> 팝업 등록 </A>
	</div>
</div>
...
<!-- 			   	<span class="td-350">시스템 점검 안내</span> -->
...
<!-- 			</li> -->
	...
	<br>
	<a class="btn btn01" onclick="javascript:delPopup();">선택 삭제</a>
		...
		<table>
			<thead>
				<tr>
					...
					<th class="td-350">제목</th>
					<th>크기 </th>
					<th>작성자 </th>
					<th class="td-170">기간 </th>
					<th>등록일/수정일 </th>
					<th>출력</th>
					<th>관리</th>
				</tr>
